...
//             <div
//               key={index}
//               className="bg-gray-800 rounded-lg shadow-lg p-6 transform transition duration-500 hover:scale-105 cursor-pointer"
//             >
//               {campaign.urlToImage ? (
//                 <img
    ...
    const fetchCampaigns = async () => {
      try {
        const response = await fetch(
          "https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=6a2ce03436f44cb992c55245e839694f"
        );
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
        const data = await response.json();
        setCampaigns(data.articles);
      } catch (error) {
        console.error(
          "Error fetching campaigns:",
          error.message
